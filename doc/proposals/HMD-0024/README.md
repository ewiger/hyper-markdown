...
more than one tool — the conformance corpus, the example trees, the
documentation, the site, the repository's own test guards — stays at the root.
...
- Anything used by more than one tool MUST stay at the repository root. That is
  `conformance/cases/` (both implementations run it), `examples/` (both lint
  it, and one launches an editor against it), `doc/`, `mkdocs.yml`, and
  `tests/`.
...
### The packaging inputs the tool does not own

The distribution's README, license, changelog, and shipped fixture are all
repository-level files, and the tool's project directory is one level down from
them.

- setuptools refuses to read a file outside the project directory: a `readme`
  of `../../README.md` fails the build outright rather than resolving. Each of
  the four MUST therefore be a symlink checked into git inside `tools/hmd`,
  pointing at the root: `README.md`, `LICENSE`, `CHANGELOG.md`, and `examples`.
- The symlinks MUST NOT be replaced by copies. A copied README is a second
  front page that drifts from the first, and the PyPI long description is
  supposed to *be* the repository's README rather than resemble it.
- Archives follow the links rather than recording them, so the sdist carries
  real bytes and the fixture still lints straight out of it.
...
- The symlinks in `tools/hmd` point *into* the repository and are followed only
  by a build run from a checkout the operator already has. They introduce no
  path the build could not otherwise read.
...
2. Give the Python tool its own project file, add the symlinked packaging
   inputs, and make the root a uv workspace root.
...
- `tools/hmd/pyproject.toml` — the distribution; `MANIFEST.in` and the four
  symlinks beside it.
...
- [`doc/DEVELOPER.md`](../../DEVELOPER.md) — the per-tool build and test
  commands.
