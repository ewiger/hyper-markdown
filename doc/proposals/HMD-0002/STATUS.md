...
| M5.28 | The top bar is a structure rather than a table of contents. Seven chapters had a tab each, so the bar listed pages and said nothing about which belong together; they are now two nested sections — `Learn` (Introduction, Tutorial, Features, Presentation), the prose read cold and in order, and `Language` (Language Specification, Namespaces, Vision), consulted afterwards and led by the normative text — beside the existing `Wiki` and `Tools`. Grouping is `nav:` nesting and nothing else: no URL moves, and the derived wiki section still skips what the authored nav places, at whatever depth it places it. The guard split in step, since a section tab carries the section's name and the first child's href: the tab bar is asserted to reach `wiki/hmd-lang-spec/` and the nav tree to still title it *Language Specification* | — | `tests/test_docs.py`, `mkdocs build --strict` |
| M5.29 | The `Tools` tab lands on a page instead of a GitHub tree. `doc/tools/index.md` is a quick start — the extension, then `pip install HyperMarkDown`, then the `mkdocs` extra, each with what it gets you — and it is the section's *index page* under `navigation.indexes`, so the tab and the sidebar heading both open it, as `Wiki` already did. The four source links stay beneath it. The book's one page about a tool became two, so the rule the nav comment states is now "installing is a page here, reading is a link out", and everything that could go stale still lives in the tools' own READMEs. Linked from the cover twice: the opening tip and the end of its `Tools` section | — | `mkdocs build --strict`, rendered `/tools/`: no red links, both install buttons |
...
- 2026-08-11: M5.28 — the nav grew two levels. Every chapter had been its own
  tab, which is a top bar that lists pages instead of naming subjects, so the
  seven of them became `Learn` and `Language`. The change is `nav:` nesting
  alone; what it cost was one guard, and only because a *section* tab shows the
  section's name over the first child's href, so asserting both against one
  anchor stopped working. The lesson is the small one: an assertion that reads
  two facts off the same element cannot say which of them moved.
- 2026-08-11: M5.29 — the `Tools` tab had led to a GitHub tree listing four
  directories, which answers *where is the source* for a reader who already knows
  what to install and abandons every other one. It leads to a quick start now.
  The nav change was one line, because `navigation.indexes` was already on and
  the pattern was already in the file: `Wiki` has had an index page since the
  book got a wiki. Worth noting against L9's instinct that the book should hold
  no page about a tool — the boundary that survives is narrower and holds better:
  *installing* is a page here, *reading* is a link out, and nothing versioned
  moved out of the tools' own READMEs.
